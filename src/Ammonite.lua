...
local LDB =
  LibStub("LibDataBroker-1.1", true):NewDataObject(
    "Ammonite!", {
      type = "data source",
      text = "Ammonite!",
      ...
      OnClick = function()
        ...
        InterfaceOptionsFrame_OpenToCategory("Ammonite")
        InterfaceOptionsFrame_OpenToCategory("Ammonite")
      end
    })
    ...
    ac = {
      ...
      args = {
        enable = {
          ...
          type = "toggle",
          set = function(info, val)
            Ammonite.db.profile.modules.ammoCounter.enabled =
              val
          end,
          get = function(info)
            return Ammonite.db.profile.modules.ammoCounter
                     .enabled
          end
        }
      }
...
function Ammonite:OnInitialize()
  ...
  AceConfig:RegisterOptionsTable("Ammonite",
                                 Ammonite.options,
                                 {"ammonite", "am"})
  self.optionsFrame =
    LibStub("AceConfigDialog-3.0"):AddToBlizOptions(
      "Ammonite", "Ammonite", "Options");
  Ammonite:Print("Version: {{VERSION}}")
end
function Ammonite:ToggleMinimapButton()
  self.db.profile.minimap.visible =
    not self.db.profile.minimap.visible
end
